...
export async function PUT(
  ...
  try {
    ...
    const { billTo, defaultPaymentProfile } = body;

    if (!billTo) {
      return NextResponse.json(
        {
          error: 'BillTo information is required',
          message: 'Please provide billing address information',
        },
        { status: 400 }
      );
    }
    ...
    const updatedProfileId = await authorizeNetService.updateCustomerPaymentProfile(
      profileId,
      paymentProfileId,
      {
        billTo,
    ...
    return NextResponse.json({
      ...
      paymentProfileId: updatedProfileId,
    });
  } catch (error) {
